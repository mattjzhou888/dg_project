...
class TS_Trader:
    ...
    def trade(self, symbol, hour):
        ...
        elif self.holding_long[symbol] and long_volume_sum < -self.size_threshold:
            self.trade_shares(symbol, "SELL", "1", time, long_volume_sum)
        elif self.holding_long[symbol] and self.trade_price[symbol] - last_price >= self.trade_stop_loss:
            self.trade_shares(symbol, "SELL", "1", time, long_volume_sum)
        ...
        # elif self.holding_short[symbol] and short_volume_sum > size_threshold:
        #     self.trade_shares(symbol, "BUYTOCOVER", "1", time, short_volume_sum)
        # elif self.holding_short[symbol] and last_price - self.trade_price[symbol] >= self.trade_stop_loss:
        #     self.trade_shares(symbol, "BUYTOCOVER", "1", time, short_volume_sum)
